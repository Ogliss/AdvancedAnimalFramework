	...
	public abstract class WorkGiver_WPGrower : WorkGiver_Scanner
	{
		...
		public override IEnumerable<IntVec3> PotentialWorkCellsGlobal(Pawn pawn)
		{
			...
			for (int i = 0; i < bList.Count; i = num + 1)
			{
				...
				bool flag = building_PlantGrower != null && this.ExtraRequirements(building_PlantGrower, pawn) && !building_PlantGrower.IsForbidden(pawn) && pawn.CanReach(building_PlantGrower, PathEndMode.OnCell, maxDanger, false, false, TraverseMode.ByPawn) && !building_PlantGrower.IsBurning();
				if (flag)
				{
				...
				if (flag2)
				{
					...
					else
					{
						...
						if (flag4)
						{
							...
							if (flag5)
							{
								bool flag6 = pawn.CanReach(growZone.Cells[0], PathEndMode.OnCell, maxDanger, false, false, TraverseMode.ByPawn);
								if (flag6)
								{
